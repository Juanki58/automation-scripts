...
def check_system_health(disk_path="/", threshold_percent=85.0):
    ...
    if used_percent > threshold_percent:
        return False, f"CRITICAL: High disk usage detected at {used_percent:.2f}%"
    return True, f"OK: Disk usage is safe at {used_percent:.2f}%"
...
if __name__ == "__main__":
    ...
    health_status, health_msg = check_system_health()
    print(health_msg)
    
    # 2. Simulate targeted backup
    ...
    # 3. Alert Dispatcher Simulation
    if not health_status or "FAILURE" in backup_msg:
